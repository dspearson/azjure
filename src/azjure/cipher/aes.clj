...
(def ^{:private true
  ...
  sbox
  "#### sbox
  Substitution box as a vector of 256 bytes.

  Use `(nth sbox (bit-and n 0xff))` to look up the substition value for n, where
  n is an unsigned byte."
...
(def ^{:private true
  ...
  invsbox
  "#### invsbox
  Inverse substitution box as a vector of 256 bytes.

  Use `(nth invsbox (bit-and n 0xff))` to look up the substition value for n,
  where n is an unsigned byte."
...
(def ^{:private true
  ...
  x2gf256
  "#### x2fg256
  Fixed field multiplication table for multiplication by 2 in
  GF(2<sup>8</sup>)"
...
(def ^{:private true
  ...
  x3gf256
  "#### x3gf256
  Fixed field multiplication table for multiplication by 3 in
  GF(2<sup>8</sup>)"
...
(def ^{:private true
  ...
  x9gf256
  "#### x9gf256
  Fixed field multiplication table for multiplication by 9 in
  GF(2<sup>8</sup>)"
...
(def ^{:private true
  ...
  x11gf256
  "#### x11gf256
  Fixed field multiplication table for multiplication by 11 in
  GF(2<sup>8</sup>)"
...
(def ^{:private true
  ...
  x13gf256
  "#### x13gf256
  Fixed field multiplication table for multiplication by 13 in
  GF(2<sup>8</sup>)"
...
(def ^{:private true
  ...
  x14gf256
  "#### x14gf256
  Fixed field multiplication table for multiplication by 14 in
  GF(2<sup>8</sup>)"
...
(def ^{:private  true
  ...
  mask
  "#### mask
  Memoization of the maskfn function"
  (memoize maskfn))
...
(def ^{:private true
  ...
  shift-in-bits
  "#### shift-in-bits
  Memoization of shift-in-bitsfn"
  (memoize shift-in-bitsfn))
...
(def ^{:private true
  ...
  inv-shift-in-bits
  "#### inv-shift-in-bits
  Memoization of inv-shift-in-bitsfh"
  (memoize inv-shift-in-bitsfn))
...
(defn- get-in-sbox
  "### get-in-sbox
  Get a byte value from the sbox (normal or inverse).  The shift argument
...
(def ^{:private true
  ...
  expand-key
  "#### expand-key
  Memoization of expand-keyfn"
  (memoize expand-keyfn))
