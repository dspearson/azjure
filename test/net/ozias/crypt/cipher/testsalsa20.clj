...
(deftest ^{:doc "Test Salsa20 stream suites"} testStream
  (testing "Stream"
    (is (= true (every? true? (map #(encryptor % :key initmap) s20s-test-vectors))))
    (is (= true (every? true? (map #(decryptor % :key initmap) s20s-test-vectors))))))

(deftest ^{:doc "Test Salsa20 counter mode suite"} testCounter
  (testing "Counter"
    (is (= true (every? true? (map #(encryptor % :key initmap :iv iv-64b) s20ctr-test-vectors))))
    (is (= true (every? true? (map #(decryptor % :key initmap :iv iv-64b) s20ctr-test-vectors))))))
