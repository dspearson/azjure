...
Evaluates to a 32-bit word."
  ...
  (bit-xor
    ...
    (nth s6 (get-byte (flip b) w0))
    (nth s7 (get-byte (flip c) w1))
    (nth s8 (get-byte (flip d) w1))
    (nth tail e)))
...
Evaluates to a vector of 4 bytes."
  ...
  [[a b c d] [w0 w1 w2 w3]]
  [(nth s5 (get-byte (flip a) w0))
   (nth s6 (get-byte (flip b) w1))
   (nth s7 (get-byte (flip c) w2))
...
(defmethod decrypt-block :cast5 [m block]
