;; ## AES Encryption/Decryption Engine
;; Defined to meet spec at [http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf](http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf)
(ns ^{:author "Jason Ozias"}
     com.ozias.net.crypt.aes)

;; Substitution box for encryption
(def Sbox
  (vector ))

(def invSbox
  (vector ))

;; The round constant word array contains the values given by <br/>
;; [x<sup>i-1</sup>,{00},{00},{00}], with x<sup>i-1</sup> being 
;; powers of x (x is denoted as {02}) in the field GF(2<sup>8</sup>)
;; (note that i starts at 1, not 0).
(def rcon 
  (vector 0x00000000 0x01000000 0x02000000 0x04000000
          0x08000000 0x10000000 0x20000000 0x40000000 
          0x80000000 0x1b000000 0x36000000 0x6c000000
          0xd8000000 0xab000000 0x4d000000 0x9a000000
          0x2f000000 0x5e000000 0xbc000000 0x63000000
          0xc6000000 0x97000000 0x35000000 0x6a000000
          0xd4000000 0xb3000000 0x7d000000 0xfa000000
          0xef000000 0xc5000000 0x91000000))

(def FFmul2
  (vector ))
  
(def FFmul3
  (vector ))

(def FFmul9
  (vector ))

(def FFmul11
  (vector ))

(def FFmul13
  (vector ))

(def FFmul14
  (vector ))

;; Takes a vector of 4 bytes (32 bits) and creates
;; one 32-bit word composed of the 4 bytes
;; e.g. [0x12 0xab 0x1f 0x3b] becomes 0x12ab1f3b
(defn bit-word [vec]
  (bit-or (bit-shift-left (nth vec 0) 24)
          (bit-shift-left (nth vec 1) 16)
          (bit-shift-left (nth vec 2) 8)
          (nth vec 3)))

;; Takes a word (32 bits, 4 bytes) and creates
;; a vector of the 4 bytes individually.
;; e.g. 0x12ab1f3b becomes [0x12 0xab 0x1f 0x3b]
(defn word-byte [word]
  (conj []
        (bit-and 0xff (bit-shift-right word 24))
        (bit-and 0xff (bit-shift-right word 16))
        (bit-and 0xff (bit-shift-right word 8))
        (bit-and 0xff word)))

;; Rotates a 32-bit word left shift bytes, placing the 
;; leftmost byte(s) in the rightmost byte positions.
;;  
;; (rotate-word-left 0x12ab1f3b 1) returns 0xab1f3b12
;; (rotate-word-left 0x12ab1f3b 2) returns 0x1f3b12ab
;; (rotate-word-left 0x12ab1f3b 3) returns 0x3b12ab1f
(defn rotate-word-left [word shift]
  (let [sft (mod shift 4)
        lshift (* 8 sft)
        rshift (- 32 lshift)
        mask (bit-and (bit-shift-left 0xFFFFFFFF rshift) 0xFFFFFFFF)]
    (if (= 0 sft)
      word
      (bit-or (bit-and (bit-shift-left word lshift) 0xFFFFFFFF)
              (bit-shift-right (bit-and word mask) rshift)))))

(defn rotate-word-right [word shift]
  (let [sft (mod shift 4)
        rshift (* 8 sft)
        lshift (- 32 rshift)
        umask (bit-and 
               (bit-shift-left 0xFFFFFFFF lshift) 0xFFFFFFFF)
        lmask (bit-and 
               (bit-shift-left 0xFFFFFFFF rshift) 0xFFFFFFFF)]
    (if (= 0 sft)
      word
      (bit-or (bit-and (bit-shift-left word lshift) umask)
              (bit-shift-right (bit-and word lmask) rshift)))))

;; Get a byte value out of the Sbox vector
;; shift argument should be a multiple of 8
(defn get-in-sbox [sbox] 
  (fn [word shift]
    (if (= 0 shift)
      (nth sbox (bit-and word 0xff))
      (bit-shift-left 
       (nth sbox 
            (bit-and (bit-shift-right word shift) 0xff)) shift))))

;; Substitues each byte (most significant first) in a word
;; with a new byte from the Sbox
(defn sub-word [word]
  (let [sfn (get-in-sbox Sbox)]
    (bit-or
     (sfn word 24)
     (sfn word 16)
     (sfn word 8)
     (sfn word 0))))

(defn inv-sub-word [word]
  (let [sfn (get-in-sbox invSbox)]
    (bit-or
     (sfn word 24)
     (sfn word 16)
     (sfn word 8)
     (sfn word 0))))

(defn get-last-nk [vec nk]
  (subvec vec (- (count vec) nk)))

(defn next-word [nk]
  (fn [vec idx]
    (let [tail (get-last-nk vec nk)
          temp (last tail)
          prev (first tail)]
      (conj vec
            (if (= (mod idx nk) 0)
              (bit-xor
               (bit-xor 
                (sub-word (rotate-word-left temp 1))
                (nth rcon (/ idx nk)))
               prev)
              (if (and (> nk 6) (= (mod idx nk) 4))
                (bit-xor (sub-word temp) prev)
                (bit-xor temp prev)))))))

(defn expand-key [key]
  (let [nb 4
        nk (/ (count key) nb)
        nr (+ nk 6)]
    (reduce #((next-word nk) %1 %2)
          (into [] (map #(bit-word %) (partition 4 key)))
          (range nk (* nb (+ nr 1))))))

(defn- print-state [label state]
  (print label)
  (println (map #(Long/toHexString %) state))
  state)

(defn transpose [matrix]
  (apply mapv vector matrix))

(defn add-round-key [state ks]
  (print-state "State (ARK): " (map bit-xor state ks)))

(defn- to-matrix [state]
  (mapv #(word-byte %) state))

(defn- to-words [matrix]
  (mapv #(bit-word %) matrix))

(defn shift-rows [state]
  (let [words (to-words (transpose (to-matrix state)))
        rotated (map #(rotate-word-left %1 %2) words (range 4))]
    (print-state "State (SR):  " 
                 (to-words (transpose (to-matrix rotated))))))

(defn inv-shift-rows [state]
  (let [words (to-words (transpose (to-matrix state)))
        rotated (map #(rotate-word-right %1 %2) words (range 4))]
    (print-state "State (ISR): "
                 (to-words (transpose (to-matrix rotated))))))
  
(defn mix-column [word]
  (conj []
        (bit-xor
         (nth FFmul2 (nth word 0))
         (nth FFmul3 (nth word 1))
         (nth word 2)
         (nth word 3))
        (bit-xor
         (nth word 0)
         (nth FFmul2 (nth word 1))
         (nth FFmul3 (nth word 2))
         (nth word 3))
        (bit-xor
         (nth word 0)
         (nth word 1)
         (nth FFmul2 (nth word 2))
         (nth FFmul3 (nth word 3)))
        (bit-xor
         (nth FFmul3 (nth word 0))
         (nth word 1)
         (nth word 2)
         (nth FFmul2 (nth word 3)))))

(defn inv-mix-column [word]
  (conj []
        (bit-xor
         (nth FFmul14 (nth word 0))
         (nth FFmul11 (nth word 1))
         (nth FFmul13 (nth word 2))
         (nth FFmul9 (nth word 3)))
        (bit-xor
         (nth FFmul9 (nth word 0))
         (nth FFmul14 (nth word 1))
         (nth FFmul11 (nth word 2))
         (nth FFmul13 (nth word 3)))
        (bit-xor
         (nth FFmul13 (nth word 0))
         (nth FFmul9 (nth word 1))
         (nth FFmul14 (nth word 2))
         (nth FFmul11 (nth word 3)))
        (bit-xor
         (nth FFmul11 (nth word 0))
         (nth FFmul13 (nth word 1))
         (nth FFmul9 (nth word 2))
         (nth FFmul14 (nth word 3)))))

;; 2a0 + 3a1 + a2 + a3
;; a0 + 2a1 + 3a2 + a3
;; a0 + a1 + 2a2 + 3a3
;; 3a0 + a1 + a2 + 2a3
(defn mix-columns [state]
  (print-state 
   "State (MC):  "
   (map #(bit-word %) 
        (map #(mix-column %) 
             (map #(word-byte %) state)))))

;; 14 11 13 9
;; 9 14 11 13
;; 13 9 14 11
;; 11 13 9 14
(defn inv-mix-columns [state]
  (print-state
   "State (IMC): "
   (map #(bit-word %)
        (map #(inv-mix-column %)
             (map #(word-byte %) state)))))

(defn sub-bytes [state]
  (print-state 
   "State (SB):  " 
   (map #(sub-word %) state)))

(defn inv-sub-bytes [state]
  (print-state
   "State (ISB): "
   (map #(inv-sub-word %) state)))

(defn cipher [ks nr]
 (fn [state round]
   (let [_ (if (> round 0) (println))
         _ (println (str "Round:       " round))
         _ (print-state "State:       " state)
         next (+ round 1)
         lower (* round 4)
         upper (* next 4)
         km (subvec ks lower upper)]
     (if (= round 0)
       (add-round-key state km)
       (if (< round nr)
         (add-round-key 
          (mix-columns
           (shift-rows 
            (sub-bytes state))) km)
         (add-round-key 
          (shift-rows 
           (sub-bytes state)) km))))))

(defn inv-cipher [ks nr]
  (fn [state round]
    (let [_ (if (< round nr) (println))
          _ (println (str "Round:       " round))
          _ (print-state "State:       " state)
          next (+ round 1)
          lower (* round 4)
          upper (* next 4)
          km (subvec ks lower upper)]
      (if (= round nr)
        (add-round-key state km)
        (if (> round 0)
          (inv-mix-columns
           (add-round-key 
            (inv-sub-bytes 
             (inv-shift-rows state)) km))
          (add-round-key 
           (inv-sub-bytes 
            (inv-shift-rows state)) km))))))

(defn encrypt-block [state key]
  (let [nb 4
        nk (/ (count key) nb)
        nr (+ nk 6)
        ek (expand-key key)]
    (into []
         (reduce #((cipher ek nr) %1 %2)
                 state (range (+ nr 1))))))

(defn decrypt-block [state key]
  (let [nb 4
        nk (/ (count key) nb)
        nr (+ nk 6)
        ek (expand-key key)]
    (into []
         (reduce #((inv-cipher ek nr) %1 %2)
                 state (range nr -1 -1)))))
